...
def show_polynomial_regression():
    ...
    price_columns_to_exclude = ['Цена кв м', 'Цена', 'Цена со скидкой', 'Изменение цены последнее', 'Изменение цены',
                               'ID ЖК', 'ID Корпуса','ЖК рус','ЖК англ','Корпус','кр Корпус','Регион','ID кв',
                               'Дата актуализации', 'Номер на этаже','Номер в корпусе','Номер секции','Адрес корп',
                               'lat','lng','Округ Направление', 'АТД','Источник','Тип корпуса','Тип кв/ап',
                               'Тип помещения','Отделка помещения','Отделка К','Договор К', 'Сдача К','Зона',
                               'Отделка текст','Старт продаж К','Изменение цены последнее','Экспозиция','Изменение цены']
    
    available_features = [col for col in available_features if col not in price_columns_to_exclude and col in analysis_data.columns]
    ...
    if final_count == 0:
        st.error("После обработки пропусков в выбранных признаках не осталось данных!")
        return
    ...
    if st.button("Обучить модель", type='primary'):
        try:
            ...
            forecast_df['Изменение, %'] = ((forecast_df['Прогноз на 2026 год'] - forecast_df['Фактическая цена']) / forecast_df['Фактическая цена'] * 100)
